...
function card(c){return `<div class="card ${norm(c.statut)}" onclick="selectAtelier('${c.id}')"><h4>${c.numero_dossier||'-'} - ${c.numero_colis||'-'}</h4><p>${c.client||'-'}</p><p>${c.longueur||'-'} × ${c.largeur||'-'} × ${c.hauteur||'-'} cm</p><span class="badge">${c.statut}</span></div>`}
let EMBALLAGE_FILTER='';
function emballageCard(c){return `<div class="card ${norm(c.statut)}" onclick="selectEmballage('${c.id}')"><h4>${c.id} • ${c.numero_dossier||'-'}</h4><p>Colis ${c.numero_colis||'-'} • ${c.client||'-'}</p><p>${c.longueur||'-'} × ${c.largeur||'-'} × ${c.hauteur||'-'} cm</p><span class="badge">${c.statut||'A créer'}</span></div>`}
async function initEmballage(){await loadEmballage('');newEmballage();document.getElementById('caisseForm')?.addEventListener('submit',saveEmballageForm);}
async function loadEmballage(filter=''){EMBALLAGE_FILTER=filter;await loadCaisses();renderEmballageList();}
function renderEmballageList(){const el=document.getElementById('emballageList');if(!el)return;const items=EMBALLAGE_FILTER?CAISSES.filter(c=>c.statut===EMBALLAGE_FILTER):CAISSES;el.innerHTML=items.map(emballageCard).join('')||'<div class="empty">Aucune demande</div>';}
function newEmballage(){const form=document.getElementById('caisseForm');if(!form)return;form.reset();document.getElementById('emballageEditId').value='';document.getElementById('emballageFormTitle').textContent='Nouvelle demande';document.getElementById('emballageSubmitBtn').textContent='Créer la demande';if(form.elements.statut)form.elements.statut.value='A créer';}
async function selectEmballage(id){const c=CAISSES.find(x=>x.id===id)||await api('/api/caisses/'+id);const form=document.getElementById('caisseForm');if(!form)return;document.getElementById('emballageEditId').value=c.id;document.getElementById('emballageFormTitle').textContent=`Détail demande ${c.id}`;document.getElementById('emballageSubmitBtn').textContent='Enregistrer les modifications';Object.entries(c).forEach(([k,v])=>{if(form.elements[k])form.elements[k].value=v??'';});}
async function saveEmballageForm(e){e.preventDefault();const form=e.target;const id=document.getElementById('emballageEditId').value;const data=Object.fromEntries(new FormData(form).entries());if(id){await api('/api/caisses/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});if(data.statut){await api('/api/caisses/'+id+'/status',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({statut:data.statut})});}await loadEmballage(EMBALLAGE_FILTER);await selectEmballage(id);}else{const res=await api('/api/caisses',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});await loadEmballage(EMBALLAGE_FILTER);newEmballage();if(res&&res.id)selectEmballage(res.id);} }
function renderKanban(){renderEmballageList();}
...
async function selectAtelier(id){SELECTED=id;const c=CAISSES.find(x=>x.id===id)||await api('/api/caisses/'+id);const debit=await api('/api/caisses/'+id+'/debit');const box=document.getElementById('atelierDetail');if(!box)return;box.innerHTML=`<div class="detail-grid"><div class="box"><h3>${c.id} • ${c.numero_dossier||'-'}</h3><p><b>Client :</b> ${c.client||'-'}</p><p><b>Chargé :</b> ${c.charge_projet||'-'}</p><p><b>Type :</b> ${c.type_caisse||'-'}</p><p><b>Bons d’entrée / nature marchandises :</b> ${c.nature_marchandises||'-'}</p><p><b>Dims int :</b> ${c.longueur||'-'} × ${c.largeur||'-'} × ${c.hauteur||'-'} cm</p><p><b>Délai :</b> ${c.delai_demande||'-'}</p></div><div class="box"><div class="form-grid"><label>Nom scieur<input id="edit_nom_scieur" value="${c.nom_scieur||''}"></label><label>Temps (en h)<input id="edit_temps_scieur" type="number" step="0.25" value="${c.temps_scieur||''}"></label><label>Nom barreur<input id="edit_nom_barreur" value="${c.nom_barreur||''}"></label><label>Temps (en h)<input id="edit_temps_barreur" type="number" step="0.25" value="${c.temps_barreur||''}"></label><label class="full">Date prévue<input id="edit_date" type="date" value="${c.date_prevue||''}"></label><label class="full">Commentaire<textarea id="edit_commentaire">${c.commentaire_atelier||''}</textarea></label></div><button class="secondary" onclick="saveAtelier()">Enregistrer</button></div></div><div class="filters"><button onclick="setStatus('A créer')">À créer</button><button onclick="setStatus('En cours')">En cours</button><button class="success" onclick="setStatus('Caisse prête')">Caisse prête</button><button class="danger" onclick="setStatus('Annulée')">Annuler</button></div><h3>Dimensions extérieures</h3>${debit.ok?`<div class="box">${debit.dims_ext.longueur} × ${debit.dims_ext.largeur} × ${debit.dims_ext.hauteur} cm</div>`:`<div class="empty">${debit.message}</div>`}<h3>Débit</h3><table class="table"><tr><th>Famille</th><th>Pièce</th><th>Qté</th><th>Longueur</th><th>Largeur</th><th>Ép.</th></tr>${(debit.lignes||[]).map(l=>`<tr><td>${l.famille}</td><td>${l.piece}</td><td>${l.quantite}</td><td>${l.longueur}</td><td>${l.largeur}</td><td>${l.epaisseur}</td></tr>`).join('')}</table>`;}
async function saveAtelier(){await api('/api/caisses/'+SELECTED,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({nom_scieur:document.getElementById('edit_nom_scieur').value,temps_scieur:document.getElementById('edit_temps_scieur').value,nom_barreur:document.getElementById('edit_nom_barreur').value,temps_barreur:document.getElementById('edit_temps_barreur').value,date_prevue:document.getElementById('edit_date').value,commentaire_atelier:document.getElementById('edit_commentaire').value})});await loadAtelier();await selectAtelier(SELECTED);}
